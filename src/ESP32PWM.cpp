...
int ESP32PWM::allocatenext(double freq)
{
	...
	Serial.println(
		"ERROR All PWM timers allocated! Can't accomodate " + String(freq) + "Hz\r\nHalting...");
	while (1)
		;
}
	...
	for (int i = 0; i < timerCount[getTimer()]; i++)
	{
		...
		if (ChannelUsed[pwm] != NULL)
			if (ChannelUsed[pwm]->getTimer() == getTimer())
			{
				...
				{
					Serial.println(
						"\tWARNING PWM channel " + String(pwmChannel) + " shares a timer with channel " + String(pwm) + "\n"
																														"\tchanging the frequency to " +
						String(freq) + " Hz will ALSO change channel " + String(pwm) + " \n\tfrom its previous frequency of " + String(ChannelUsed[pwm]->myFreq) + " Hz\n"
																																								   " ");
